...
Phantom_4D_T2 =  reshape(Phantom_4D_T2,[size(reshapedPhantom,3), length(MyInfo.Times)]);
...
reshapedPhantom = reshape(Phantom_3D, [size(Phantom_3D,1)*size(Phantom_3D,2)*size(Phantom_3D,3),1]);
Phantom_4D_T2star =  reshape(Phantom_4D_T2star, [size(reshapedPhantom,3), length(MyInfo.Times)]);
